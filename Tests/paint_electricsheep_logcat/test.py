...
class Test(libgreenminer.AndroidTest):
	...
	def after(self, run):
		...
		# Get PID
		self.pid = run.phone.shell("ps | grep com.electricsheep.paint").split()[1]
		# Get Log
		run.phone.shell("logcat -d | grep -e "+ self.pid.strip() +" -e com.electricsheep.paint > /sdcard/logcat.txt")
