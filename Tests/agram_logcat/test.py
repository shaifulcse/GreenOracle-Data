...
class Test(libgreenminer.AndroidTest):
	...
	def after(self, run):
		...
		# Get PID
		self.pid = run.phone.shell("ps | grep us.achromaticmetaphor.agram").split()[1]
		# Get Log
		run.phone.shell("logcat -d | grep -e "+ self.pid.strip() +" -e us.achromaticmetaphor.agram > /sdcard/logcat.txt")
