...
class Test(libgreenminer.AndroidTest):
	...
	def after(self, run):
		...
		# Get PID
		self.pid = run.phone.shell("ps | grep de.onyxbits.sensorreadout").split()[1]
		# Get Log
		run.phone.shell("logcat -d | grep -e "+ self.pid.strip() +" -e de.onyxbits.sensorreadout > /sdcard/logcat.txt")
