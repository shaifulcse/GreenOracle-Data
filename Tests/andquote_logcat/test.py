...
class Test(libgreenminer.AndroidTest):
	...
	def after(self, run):
		...
		# Get PID
		self.pid = run.phone.shell("ps | grep net.progval.android.andquote").split()[1]
		# Get Log
		run.phone.shell("logcat -d | grep -e "+ self.pid.strip() +" -e net.progval.android.andquote > /sdcard/logcat.txt")
